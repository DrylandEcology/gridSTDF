...
Panel1 <- ggplot() +
  ...
  geom_ribbon(data = PPTdata, aes(x = Date, y = ppt_rollsum.med, 
                                                 ymin = ppt_rollsum.10, ymax = ppt_rollsum.90,  alpha=0.1), fill = 'lightgrey') +
  geom_line(data = PPTdata, aes(Date, ppt_rollsum.med),  color = 'black') +
...
Panel2 <- ggplot() + 
  ...
  geom_ribbon(data = PPTdata, aes(Date, ymin = ppt_roll.10.diff, 
                                               ymax = ppt_roll.90.diff, alpha=0.01), fill = 'lightgrey') +
  geom_line(data = PPTdata, aes(Date, ppt_roll.10.diff), size = .1, color = 'black') +
  geom_line(data = PPTdata, aes(Date,  ppt_roll.90.diff), size = .1, color = 'black') +
