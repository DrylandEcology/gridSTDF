...
# Make Table of means
AllPPTMeans_CF <- cbind( MVGen_PPT_CF_Mean = pptGenAnomsMean$MVGenMean, 
                        NWS_PPT_CF = PPTAnoms$Anom_CF)
AllPPTMedians_CF <- cbind( MVGenMed = pptGenAnomsMedian$MVGenMed, 
                         NWS_PPT_CF = PPTAnoms$Anom_CF)
...
p3Means <- p3 +  
  ...
  #means
  geom_vline(data = pptGenAnomsMean, aes(xintercept = MVGenMean, color = 'MV Generated Values'), size = 1.3)+
 ...
 # geom_vline(data = pptAnalyticalDensityMean, aes(xintercept = AnalyticalMean_CF), color = 'blue', size = 1.5) +
  geom_vline(data = PPTAnoms[1:12,], aes(xintercept = Anom_CF, color = 'NWS Data'),  size = 1.3,  lty = 'dotted') +
